...
*/
	function create_IP_Log($ip,$site_fk,$attempted_access_to,$user_fk=FALSE)
	{
		if($user_fk == FALSE){
			$user_fk = NULL;
		}
		...
		$sql = "INSERT INTO IP_LOG
			  (IP_ADDRESS,SITE_FK,CREATED_AT,ATTEMPTED_ACCESS_TO,USER_FK) 
			  VALUES ('$ip',$site_fk,'$attempted_access_to',$user_fk) " ;
...
*/
	function read_IP_Log($ip_log_id=FALSE,$ip_address=FALSE,$site_fk=FALSE,$attempted_access_to,$user_fk=FALSE)
	{
...
/** 
  ...
  *			echo $array_data['id'];
*/
	function read_values_IP_Log($ip_log_id=FALSE,$ip_address=FALSE,$site_fk=FALSE,$attempted_access_to,$user_fk=FALSE)
	{
		$sql = ' SELECT '.COLUMNS_IP_LOG.' FROM IP_LOG il ';				$sql.= " WHERE 0=0 ";		if($ip_log_id != FALSE){			$sql.= " AND il.IP_LOG_ID = '$ip_log_id' ";		}				if($ip_address != FALSE){			$sql.= " AND il.IP_ADDRESS = '$ip_address' ";		}		if($site_fk != FALSE){			$sql.= " AND il.SITE_FK = '$site_fk' ";		}		if($attempted_access_to != FALSE){			$sql.= " AND il.ATTEMPTED_ACCESS_TO = '$attempted_access_to' ";		}		if($user_fk != FALSE){			$sql.= " AND il.USER_FK = '$user_fk' ";		}				$result = $_SESSION['QUERY']($_SESSION['connection'],$sql);		
		// error reporting 
		if($result === false) 
